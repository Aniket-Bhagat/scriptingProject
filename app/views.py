...
from wtforms import Form, StringField, TextAreaField, PasswordField, BooleanField, StringField, SubmitField
...
def index(page=1):
  ...
  questions = Questions.query.order_by(Questions.timestamp.desc()).paginate(page, app.config['POSTS_PER_PAGE'], False)
  return render_template('index.html', title="Welcome", questions = questions)
  ...
  if(request.method=='POST' and request.form['question_keyword']):
    ...
    for question in questions:
        if(question.title and keyword.lower() in question.title.lower()):
            ...
            body[question.question_id] = question.body
    ...
    if(flag==0):    
      return render_template('results.html',title="Search Result",nomatches=True)
    else:      
      return render_template('results.html',title="Search Result", titles=titles, body=body, links=links,tags=tags)
  else:
    return redirect(url_for('index'))
...
def answers(id):
  ...
  return render_template('question.html',title="Question", question=question, postedby=username, answers=answers,commenters=commenters, 
...
def showQuestion(question_id):
  ...
  question = Questions.query.filter_by(question_id = question_id).all()
  if not question:
    return redirect(url_for('index'))
  ...
  enabled = True
  if (session or session['loggedin']) and len(Answer.query.filter_by(question_id = question_id, user_id = g.user.user_id).all()):
    enabled = False
  return render_template('question.html', title = question[0].title, questions = question, answers = answers, enabled = enabled, comments = comments)
...
def profile(uid=None):
  if session and session['userid']:
    ...
    return render_template('profile.html', title='Profile', user=user, reputation=reputation)
  else :
...
@app.route("/search_by_tag/<tag>", methods=['GET','POST'])
def searchByTag(tag):
  if(request.method=='POST' and request.form['tag1']):
  ...
  if(flag==0):    
    ...
    return render_template('search_by_tag.html',titles=titles, title='Search by tag', body=body, links=links, tags=tags_q)  
